...
/*   Updated: 2019/12/11 15:13:16 by vlaroque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point		trnspoint(t_data *d, t_ptdata ptdata)
{
	...
	d->y2d = -sin(d->w) * sin(d->a) * d->x - cos(d->w)
		* sin(d->a) * d->y + cos(d->a) * d->z;
...
int			printgrid_xmax_ymax(t_data *data)
{
	...
	while (i >= 0)
	{
		if (i % data->tab.x_max != 0)
			seg(data, data->tab.table[i], data->tab.table[i - 1]);
		if ((i / data->tab.x_max) != 0)
			seg(data, data->tab.table[i],
					data->tab.table[i - data->tab.x_max]);
		i--;
	}
	return (1);
}
...
int			printgrid_zero(t_data *data)
{
	...
	{
		if (i % data->tab.x_max != data->tab.x_max - 1)
			seg(data, data->tab.table[i], data->tab.table[i + 1]);
		if ((i / data->tab.x_max) != (data->tab.y_max - 1))
			seg(data, data->tab.table[i],
					data->tab.table[i + data->tab.x_max]);
		i++;
	}
